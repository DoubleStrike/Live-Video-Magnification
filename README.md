...
The reason for this fork is to support any config changes to build with modern Visual Studio (not just VSCode) and to provide usable binaries.  As of 11/20/2025, the build system is fully working under VS 2022! You will need to have OpenCV downloaded along with Qt, and set the paths in the appropriate environment variables.

**Non-developers can just download the zip file and extract and run - please report any issues!**
